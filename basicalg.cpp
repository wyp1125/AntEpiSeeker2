...
void pathway::input_data(char* path)
{
    ...
    while(!in.eof())
    {
        ...
        while(!values.eof())
        {
           ...
           if(weighted==1)
           {
           itp=snp2pwn.find(it->second);
           if(itp!=snp2pwn.end())
           itp->second+=1.0;
           else
           snp2pwn[it->second]=1.0;
           }
           }
        }
        if(one_pw.size()>1)
...
void pathway::sort_pw(double cutoff)
{
    ...
    if(weighted==1)
    {
    map<int,double>::iterator itq;
    for(k=0;k<j;k++)
    {
    itq=snp2pwn.find(id[i][k]);
    temp[k]=SNPdata.pheromone[id[i][k]]/itq->second;
    }
    }
    else
    {
